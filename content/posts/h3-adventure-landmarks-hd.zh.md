...
复查发现此前的一些重绘为了增加细节而改变了地标类型。`AVXALTAR` 重新回到低矮的献祭石台、后方残拱与红色仪式台面；`AVXTHRN` 重新回到岩基上紧凑、不规则的赭石城堡；`AVXSHYD0` 重新回到带起吊木柱的开放式木制下水架，而不是带仓库的大码头。绿、红、蓝方尖碑也重新回到小石座上的单色碑体。以上均只替换 2×、3×、4× 的单帧主体，原始画布、阴影、覆盖层和对象行为均保持不变。

![原始底图与最终忠实度更正](/images/h3-environment-hd/fidelity-corrections-compare.png)

饮水点（`AVXWTRH0`）也从过大的繁茂池塘收回到原作的低矮浅池：左侧岩堆、枯枝、中央石块和右侧窄石岸均被保留。市场（`AVXMKTB0`）同样回到中央矮棚与两侧小布棚的布局，不再是多屋顶的大型市集。草地水井（`AVXWELG0`）也保留原作的小型石井、短木柱与低矮瓦顶。闹鬼墓园的复查还发现它有八帧主体。没有用静态图覆盖它；该项更正必须使用已暂缓的 Blender 动画流程。路边酒馆的忠实草稿也在登记前被拒绝：预检发现 `AVXTVRN0` 同样有八帧主体，因此没有接入游戏。
...
<s>此前的巨型红木虽然更清晰，却仍像一棵普通树；这次将它重做为可一眼辨认的瞭望塔：外置螺旋梯、环形观景台、瞭望亭、望远镜和旗帜都进入主体轮廓。</s>

这种做法为了醒目而改变了地标身份。最终版 `AVXREDW` 回到原作的细长巨型红木：底部外露根系、一根极高的主干、少量侧枝，以及树梢处带宽大斜向浅色木屋顶的小型瞭望台。它是建在巨树上的瞭望点，不是独立的奇幻高塔。只替换 2×、3×、4× 的单帧主体；原始画布、阴影、覆盖层和对象行为继续保留。

![原始底图与忠实最终巨型红木重绘](/images/h3-environment-hd/redwood-faithful-final-compare.png)
