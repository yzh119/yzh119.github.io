...
饮水点（`AVXWTRH0`）也从过大的繁茂池塘收回到原作的低矮浅池：左侧岩堆、枯枝、中央石块和右侧窄石岸均被保留。闹鬼墓园的复查还发现它有八帧主体。没有用静态图覆盖它；该项更正必须使用已暂缓的 Blender 动画流程。路边酒馆的忠实草稿也在登记前被拒绝：预检发现 `AVXTVRN0` 同样有八帧主体，因此没有接入游戏。
