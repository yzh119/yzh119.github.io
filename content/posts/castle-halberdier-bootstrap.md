...
The Crusader test package is now installed locally as a separate `castle-creature-animations` mod. It contains 76 frames at each of 1× and 2× resolution. All 340 installed files match the package, and validation against the original animation groups reports no errors or warnings. A fresh client confirms that the mod loads. Battle playback remains unverified, including movement-to-stop transitions, repeated attacks, shadows and the creature information panel. Other Castle creatures still use their original artwork in the game.
