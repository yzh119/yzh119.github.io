...
**AVLR** 岩脊系列现已完整覆盖：粗岩、地下钟乳石、雪地形态、草地与荒地变体、带浪花的海岸礁岩，以及关联的静态树木。~~本轮还补齐了小型静态熔岩池、湿地树群、岸边树与浮叶水面。源目录审计确认 AVLR、AVLL、AVLW、AVLY 与 AVLK 均不再有未增强的原图。~~ 随后的帧审计发现熔岩池与浮叶水面实际为动画资源，故已恢复原版序列，待 Blender 模型与完整渲染循环完成后再替换。静态的湿地树群和岸边树继续保留；AVLR、AVLW 与 AVLY 的源目录审计无未增强原图。
